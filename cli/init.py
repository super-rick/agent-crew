...
Usage:
    agent-crew init              # Create config in current directory
    agent-crew init --dir /opt   # Create config in specific directory
    agent-crew init --force      # Overwrite existing files without prompting
"""
...
def init_command(target_dir, force):
    ...
    next_steps = Panel.fit(
        "\n".join([
            ...
            "   [dim]agent-crew write generate --topic \"Your Topic\" --style technical[/dim]",
            "",
            "[bold]3.[/bold] Explore all commands:",
            "   [dim]agent-crew --help[/dim]",
        ]),
